...
function ourAddShard(setname,port) {
	...
	res = rs.initiate(
		{
			"_id" : setname,
			"members" : [
				{ _id: 0, host: "Anyis-MacBook-Pro.local:" + port },
				{ _id: 1, host: "Anyis-MacBook-Pro.local:" + (port+1) },
				{ _id: 2, host: "Anyis-MacBook-Pro.local:" + (port+2) }
			]
		}
	);
	check();
	...
	print("adding shard...");
	db = mongosConn;
	res = sh.addShard(setname+"/Anyis-MacBook-Pro.local:"+port);
	check();
